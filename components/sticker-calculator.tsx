...
// New Sticker Mule-style pricing
const RATE = 120 // $/m²
const BREAKS = [50, 100, 200, 300, 500, 1000, 2000, 3000, 5000, 10000]

// Anchor curves (decimal discounts) for specific sizes
const CURVES = {
  2500: [0.0, 0.4, 0.61, 0.68, 0.75, 0.81, 0.85, 0.86, 0.88, 0.9], // 50×50mm (area: 2500mm²)
  5625: [0.0, 0.35, 0.54, 0.62, 0.68, 0.75, 0.79, 0.81, 0.83, 0.85], // 75×75mm (area: 5625mm²)
  10000: [0.0, 0.31, 0.49, 0.56, 0.63, 0.69, 0.74, 0.76, 0.78, 0.81], // 100×100mm (area: 10000mm²)
  15625: [0.0, 0.28, 0.44, 0.51, 0.58, 0.64, 0.7, 0.72, 0.75, 0.76], // 125×125mm (area: 15625mm²)
}

const FLOORS = [0.0, 0.2, 0.3, 0.36, 0.42, 0.48, 0.52, 0.54, 0.56, 0.58]
const BIG_DECAY_K = 0.12
...
function getCurve(area: number) {
  const keys = Object.keys(CURVES)
    .map(Number)
    .sort((a, b) => a - b)

  if (area <= keys[0]) return CURVES[keys[0] as keyof typeof CURVES]

  for (let i = 0; i < keys.length - 1; i++) {
    if (area <= keys[i + 1]) {
      const t = (area - keys[i]) / (keys[i + 1] - keys[i])
      return lerpArr(CURVES[keys[i] as keyof typeof CURVES], CURVES[keys[i + 1] as keyof typeof CURVES], t)
    }
  }

  const scale = Math.pow(area / keys[keys.length - 1], -BIG_DECAY_K)
  return CURVES[keys[keys.length - 1] as keyof typeof CURVES].map((d, i) => Math.max(d * scale, FLOORS[i]))
}

function getDiscount(qty: number, area: number) {
  const cv = getCurve(area)
  let idx = 0
  for (let i = 0; i < BREAKS.length; i++) {
    if (qty >= BREAKS[i]) idx = i
    else break
  }
  return { pct: cv[idx], idx }
}

function priceSticker({ widthMm, heightMm, qty }: { widthMm: number; heightMm: number; qty: number }) {
  const w = Math.max(20, Math.min(1000, widthMm))
  const h = Math.max(20, Math.min(1000, heightMm))
  const q = Math.max(1, qty)

  const areaMm2 = w * h
  const areaM2 = areaMm2 / 1e6
  const base = areaM2 * q * RATE
  const { pct } = getDiscount(q, areaMm2)
  ...
  return {
    ...
    unitPrice: Number((total / q).toFixed(4)),
    savePct: Math.round(pct * 100),
    nextTier: getAddMoreHint(w, h, q),
  }
}

function getAddMoreHint(widthMm: number, heightMm: number, qty: number) {
  const areaMm2 = widthMm * heightMm
  const cv = getCurve(areaMm2)

  const nextIdx = BREAKS.findIndex((b) => b > qty)
  if (nextIdx === -1) return null

  const { idx: curIdx } = getDiscount(qty, areaMm2)
  const toPct = cv[nextIdx]

  return {
    nextQty: BREAKS[nextIdx],
    addMore: BREAKS[nextIdx] - qty,
    nextSavePct: Math.round(toPct * 100),
  }
}

const qtyTiers = [50, 100, 200, 300, 500, 1000, 2000, 3000, 5000, 10000]
